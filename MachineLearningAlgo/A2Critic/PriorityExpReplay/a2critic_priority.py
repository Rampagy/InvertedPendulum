...
class A2CAgent:
    def __init__(self, state_size, action_size):
        ...
        # These are the hyper parameters for the Memory Replay
        self.epsilon = 0.01 # zero offset
        self.alpha = 0.60 # prioritization rate
    ...
    def replay_memory(self, state, action, reward, next_state, done):
        ...
        priority = float(abs(advantage) + self.epsilon)
    ...
    # Compute probability values for each TD error in x
    def compute_priority(self, x):
        transition_priority = np.power(x, self.alpha) # transition priority
        return (transition_priority / transition_priority.sum()) # probabilities
    ...
    def train_model(self):
        ...
        td_errors = np.float32([td_err for td_err, _, _, _, _, _ in self.memory])
        probability = self.compute_priority(td_errors)
        samples = np.random.multinomial(n=batch_size, pvals=probability)
        idxs = np.where(samples>0)

        mini_batch = []
        for idx in idxs[0]:
            for _ in range(samples[idx]):
                mini_batch.append(self.memory[idx] + (idx,))
        ...
        for i in range(batch_size):
            _, state, action, reward, next_state, done, idx = mini_batch[i]
            value = self.critic.predict(state)[0]
            next_value = self.critic.predict(next_state)[0]
        ...
        for i in range(batch_size):
            ...
            # bigger is higher priority
            priority = float(abs(advantage) + self.epsilon)
            self.memory[idx] = (priority, state, action, reward, next_state, done)
